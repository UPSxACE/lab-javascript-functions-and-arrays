...
function averageWordLength(arrStrings) {
  ...
  arrStrings.forEach((str) => {
    sumLengths += str.length;
  });

  return sumLengths / arrStrings.length;
}
...
function howManyTimes(arrWords, wordToCount) {
  let count = 0;
  arrWords.forEach((word) => {
    if (word === wordToCount) {
      count++;
...
function greatestProduct(mtrx) {
  ...
  if (horizontalSpaces >= 4 && verticalSpaces >= 4) {
    ...
    // reverse horizontal diagonals
    for (let i = 0; i < horizontalDiagonals; i++) {
      // initial values
      const diagonalLowerCoords = [lastRow, lastColumn - 3];
      const diagonalHigherCoords = [lastRow - 3, lastColumn];
    ...
    // reverse vertical diagonals
    for (let i = 0; i < verticalDiagonals; i++) {
      // initial values
      const diagonalLowerCoords = [0, 0];
      const diagonalHigherCoords = [lastRow - 3, 3];
